...
R"(Game 1: 4 green, 7 blue; 2 blue, 4 red; 5 blue, 2 green, 2 red; 1 green, 3 red, 9 blue; 3 green, 9 blue; 7 green, 2 blue, 2 red
...
//representation of a cube and it's colours
//friend func to extract 'cube' obj from input stream
struct Cube {
...
struct Game {
    int id;
    std::unordered_map<Cube::Color, int> maximum;
    friend std::istream &operator>>(std::istream &, Game &);
};

//implemetation of friend func for Cube >> operator
std::istream &operator>>(std::istream &is, Cube &cube) {
    //get num of cubes
    if(not(is >> cube.count)) {
        return is;
    }
    ...
    //get color from first letter, ignore the rest
    switch(is.peek()) {
        case 'b':
...
//implemetation of friend func for Game >> operator
std::istream &operator>>(std::istream &is, Game &game) {
    //make sure it starts with 'G' and ignore "Game " text
    if(is.peek() != 'G') {
    ...
    //ignore the ':' after the id
    is.ignore(1); 

    //read the cubes of the current game
    game.maximum.clear();
    Cube cube;

    while(is >> cube) {
        game.maximum[cube.color] = std::max(game.maximum[cube.color], cube.count);

        auto delim = is.get();

        //get next set of cubes
        if(delim == ',' || delim == ';') {
            continue;
        }
...
int sumPossibleGames(std::span<const char> games) {
    std::ispanstream iss(games);
    
    return std::ranges::fold_left(
      std::views::istream<Game>(iss), 0z, [](int64_t acc, auto &game) {
        if (game.maximum[Cube::RED] <= 12 && game.maximum[Cube::GREEN] <= 13 &&
            game.maximum[Cube::BLUE] <= 14)
          return acc + game.id;
...
int main(){
    std::cout << sumPossibleGames(input);
